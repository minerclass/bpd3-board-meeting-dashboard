# BPD3 Board Meeting Dashboard

This repository hosts a standalone, browser-based dashboard for Beach Park School District 3 board meeting information for the 2025-26 school year.

## Contents

- `index.html` - the complete dashboard, including timeline, category filters, meeting cards, links to public BPD3 board resources, and local page interactions.

## Publishing

This site is designed for GitHub Pages.

1. Use the `main` branch.
2. Set GitHub Pages to deploy from the repository root.
3. Open the published Pages URL after deployment to confirm the dashboard loads correctly.

## Privacy Notes

The dashboard is a static page. It links to public BPD3 resources and does not submit meeting notes, searches, or interactions to a server.
